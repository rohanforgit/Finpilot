...
import { Layers, Mail, Key, Sparkles, AlertCircle, ArrowRight, Check } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    if (!isLive) {
      ...
      setLoading(false);
      setStep("otp");
      setSuccessMsg("Offline Sandbox: OTP simulated. Enter any 6 digits.");
      return;
    }
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    if (!isLive) {
      // Offline fallback: Verify OTP simulation
      await new Promise(resolve => setTimeout(resolve, 1000));
      setLoading(false);
      document.cookie = `finpilot-user-id=demo-user-id; path=/; max-age=${30 * 24 * 60 * 60}`;
    ...
    try {
      ...
      if (error) {
        ...
        document.cookie = `finpilot-user-id=${data.session.user.id}; path=/; max-age=${30 * 24 * 60 * 60}; SameSite=Lax; Secure`;
        router.push("/dashboard");
      } else {
  ...
  const handleGoogleLogin = async () => {
    ...
    if (!isLive) {
      setErrorMsg("Google OAuth requires live Supabase configurations. Using Offline Sandbox Demo Mode.");
      setLoading(true);
  ...
  return (
    <main className="min-h-screen bg-[#09090B] flex flex-col items-center justify-center p-6 text-white select-none">
      ...
      <div className="w-full max-w-md bg-[#111113] border border-border/40 p-8 rounded-2xl shadow-2xl relative overflow-hidden space-y-8">
        ...
        {/* Status Messages */}
        {errorMsg && (
          <div className="bg-error/15 border border-error/30 p-3 rounded-lg flex items-start gap-2.5 text-xs text-error animate-fadeIn">
        ...
        {successMsg && (
          <div className="bg-success/10 border border-success/20 p-3 rounded-lg flex items-start gap-2.5 text-xs text-accent animate-fadeIn">
          ...
          {step === "email" ? (
            ...
            >
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 bg-white text-[#09090b] font-bold rounded-xl text-sm hover:bg-white/80 transition duration-300 flex items-center justify-center gap-1.5 cursor-pointer"
              >
            ...
            >
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 bg-accent text-[#09090b] font-bold rounded-xl text-sm hover:bg-accent/80 transition duration-300 flex items-center justify-center gap-1.5 cursor-pointer"
              >
        ...
        <div className="relative flex py-2 items-center">
          ...
          <span className="flex-shrink mx-4 text-gray-500 text-[10px] uppercase font-semibold">Or SSO Authenticate</span>
          <div className="flex-grow border-t border-border/30"></div>
        </div>
        ...
        <button
          ...
          className="w-full py-3 bg-[#111113] border border-border/50 rounded-xl text-xs font-semibold text-gray-300 hover:text-white hover:bg-black/60 transition duration-200 flex items-center justify-center gap-2 cursor-pointer"
        >
          {/* Google Logo SVG */}
        ...
        <div className="text-[10px] text-gray-500 text-center flex items-center justify-center gap-1.5 bg-black/20 p-2.5 rounded-lg border border-border/20">
          ...
          <span>{isLive ? "Authentication secured via live Supabase service." : "Local Offline: Supabase not configured. Using sandbox defaults."}</span>
        </div>
      </div>
    </main>
  );
